        ...
        layui.use('table', function(){
          ...
          //第一个实例
            var talbleIns = table.render({
                elem: '#departmentList',
                //height: 315,
                url: '${url}administratorServlet?type=listDepartmentLimit',//数据接口   
                cellMinWidth: 160,
                cols: [[ //表头
                  {type: 'checkbox', width: 40},
                  {field: 'dId', title: '编号', sort: true, align: 'center'},
                  {field: 'dName', title: '院系名', align: 'center'},
                  {field: 'dAddress', title: '地址', align: 'center'}, 
            ...
            table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'del') {
                    layer.confirm('确定要删除吗？', function(index){
                       obj.del(); 
                       layer.close(index);
                       var url = "${url}administratorServlet?type=deleteDepartment&dId="+data.dId;
                       $.post(url,{"dId":data.dId}, function(message){
                           layer.msg(message);
                           talbleIns.reload({
                               url: '${url}administratorServlet?type=listDepartmentLimit'
                           });
                       }, "text");                     
                    });
                    return;
                ...
                //编辑操作
                if (layEvent === 'edit') {
                    window.location.href = "${url}administratorServlet?type=getDepartmentBydId&dId="+data.dId;              
                  return ;
                }
              });           
            ...
            var $ = layui.$, active = {
                getCheckData: function(){ //获取选中数据
                    ...
                    layer.confirm('确定要删除吗？', function(index){
                        ...
                        for (var i = 0; i < data.length; i++){
                            ...
                            var url = "${url}administratorServlet?type=deleteDepartment";  
                            $(document).queue("post", 
                           		$.ajax({
                           		    url: url,
                           		    data: {"dId":department.dId}, 
                           		    type:"post",
                           		    dataType: "text",
                ...
                editStudent: function(){
                    ...
                    data = checkStatus.data;
                    if (data.length <= 0){
                        layer.alert("请选择要编辑的院系");
                        return ;
                    }
                    if (data.length > 1){
                        layer.alert("一次只能编辑一个院系");
                        return ;
                    }
                    window.location.href = "${url}administratorServlet?type=getDepartmentBydId&dId="+data[0].dId;
                },
            };
